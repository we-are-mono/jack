...
// jack-plugin-monitoring is a Jack plugin that provides system and network monitoring.
...
func main() {
	...
	log.SetPrefix("[jack-plugin-monitoring] ")

	log.Println("Starting monitoring plugin...")

	// Create the RPC provider with CLI command support
	provider := NewMonitoringRPCProvider()
